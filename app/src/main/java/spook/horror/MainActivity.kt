...
@Composable
private fun LabScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        result?.let { r ->
            ...
            StepCard(
                stepNumber = 5,
                title = "OAT byte-patch (multi-dex)",
                subtitle = "For EACH classesN.dex: dex_file_location_data → target install path, dex_file_location_checksum → installed target's classesN.dex CRC32",
            ...
            StepCard(
                ...
                subtitle = "VDEX header carries its OWN CRC array that ART cross-checks against the OAT — patch both or ART rejects both. Offset found by searching for the picked CRCs (version-agnostic; v027 stores them at 0x3c), 4 bytes per dex, natural order.",
...
private fun HeroCard() {
    Card(
        ...
        Column(Modifier.padding(16.dp)) {
            ...
            Spacer(Modifier.height(4.dp))
            Text(
                "Understanding how LuckyPatcher recompiles a DEX into an OAT at runtime, " +
                    "and the Sabanal path-padding + CRC32 patching that makes ART accept the result.",
...
private fun MechanismCard() {
    SectionCard(title = "The mechanism") {
        Text(
        ...
        BulletLine("4", "Multi-dex OAT patch: for EACH classesN.dex, swap dex_file_location_data → installed sourceDir and dex_file_location_checksum → installed classesN.dex CRC32")
        BulletLine("5", "VDEX patch: locate the dex_checksums array by searching for the picked CRC32s (version-agnostic — no hardcoded offset) and overwrite with the installed CRC32s — ART cross-checks OAT vs VDEX")
...
private fun ActionsCard(
    ...
    SectionCard(title = "Run") {
        Text(
            "Pick an APK whose package is INSTALLED on this device. We resolve its package " +
                "name → installed sourceDir, compile a fresh OAT, patch its header, and drop the " +
                "OAT into the installed app's own oat/<isa>/ folder. Requires root.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Spacer(Modifier.height(12.dp))
        Button(onClick = onPick, enabled = !running, modifier = Modifier.fillMaxWidth()) {
            Text(if (running) "running…" else "Pick APK & compile")
        }
    }
}
